...
Підготувати клієнт для користувача
  ...
  Run Keyword If  '${username}' != 'opentender_Viewer_auction'  Run Keywords
  ...  Login  ${username}
...
Завантажити ілюстрацію
  ...
  opentender.Завантажити документ   ${username}  ${filepath}  ${tender_uaid}  True
...
Задати запитання на тендер
  ...
  opentender.Задати питання  ${username}  ${tender_uaid}  ${question}

Задати запитання на предмет
  [Arguments]  ${username}  ${tender_uaid}  ${item_id}  ${question}
  opentender.Задати питання  ${username}  ${tender_uaid}  ${question}  ${item_id}
...
Отримати документ
  ...
  opentender_download_file   ${url}  ${file_name}  ${OUTPUT_DIR}
  [return]  ${file_name}
...
Отримати інформацію із документа по індексу
  ...
  ${doc_value}=  convert_string_from_dict_opentender  ${doc_value}
  [return]  ${doc_value}
...
Опублікувати Пропозицію
  ...
  ...  Go To  http://test-eauction.open-tender.com.ua/bids/send/${url.split('?')[0].split('/')[-1]}?token=465
  ...  ELSE  Go To  http://test-eauction.open-tender.com.ua/bids/decline/${url.split('?')[0].split('/')[-1]}?token=465
  Go To  ${url}
...
Завантажити фінансову ліцензію
  [Arguments]  ${username}  ${tender_uaid}  ${filepath}
  opentender.Завантажити документ в ставку  ${username}  ${filepath}  ${tender_uaid}

Змінити документ в ставці
  [Arguments]  ${username}  ${tender_uaid}  ${path}  ${docid}
  opentender.Завантажити документ в ставку  ${username}  ${path}  ${tender_uaid}
...
Отримати посилання на аукціон для учасника
  ...
  Execute Javascript  window['url'] = null; $.get( "http://test-eauction.open-tender.com.ua/seller/tender/updatebid", { id: "${current_url.split("/")[-1]}"}, function(data){ window['url'] = data.data.participationUrl },'json');
